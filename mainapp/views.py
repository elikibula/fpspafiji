...
def home(request):
    ...
    return render(request, 'home.html', {'featured_news': featured_news, 'featured_photo_news': featured_photo_news, 'latest_news': combined[:6], 'events': _safe_list(Event.objects.filter(start__gte=timezone.now()).order_by('start')[:6]), 'areas': districts, 'districts': districts})
